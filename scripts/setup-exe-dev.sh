...
# Disable fish's terminal capability query. The exe.dev web/mobile terminal
# does not respond to it, which otherwise stalls fish startup for ~10s. This is
# a fish feature flag, so it must be set as a universal variable (config files
# are sourced too late to change it).
if command -v fish >/dev/null; then
	# shellcheck disable=SC2016 # $fish_features is a fish variable, not a shell one
	if ! fish -c 'contains -- no-query-term $fish_features' 2>/dev/null; then
		fish -c 'set -Ua fish_features no-query-term'
		echo "disabled fish terminal query (no-query-term)"
	else
		echo "fish terminal query already disabled"
	fi
fi
